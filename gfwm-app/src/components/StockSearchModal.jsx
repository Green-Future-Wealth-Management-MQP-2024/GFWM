...
const StockSearchModal = ({ isOpen, onClose, stocks, currentStocks, onAddToPortfolio, onRemovePortfolio, onClickStock, selectedStocks, setSelectedStocks, handleSelectStock }) => {
...
useEffect (() => {
  ...
  const categories = [
    ...
    { name: 'Top ESG', key: 'esg' },
    { name: 'Top Environment', key: 'environment' },
    { name: 'Top Emissions', key: 'emissions' },
  ...
  const filteredStocks = stocks.filter(stock => {
    const matchesSearchTerm = stock.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
  ...
  const handleAddSelected = () => {
    if (!window.confirm(`Are you sure you want to add the selected ${selectedStocks.length} stock(s) to your portfolio?`)) {
      return;
    }
    const stocksToAdd = stocks.filter(stock => selectedStocks.includes(stock.ticker));
  ...
  const handleRemoveSelected = () => {
    if (!window.confirm(`Are you sure you want to remove the selected ${selectedStocks.length} stock(s) from your portfolio?`)) {
      return;
    }
  ...
  <div className="flex items-center mb-3">
          ...
          <button onClick={() => setSelectedCategory('all')} className="ml-2 text-xs text-gray-500 hover:underline  hover:opacity-75">
            Clear Filter</button>
        </div>
          ...
          <table className="min-w-full bg-white mb-2 text-sm">
            <thead className='sticky top-0 bg-white z-10'>
              <tr>
                ...
                <th className="py-1 px-2 border-b-2 border-gray-300 text-left leading-4 text-gray-600 tracking-wider">Status</th>
            ...
            <tbody>
              {sortedStocks.map((stock, index) =>{
                ...
                <tr key={stock.ticker} className="hover:bg-gray-100  ">
                  ...
                  <td className="border-b text-right pl-2 border-gray-300 cursor-pointer " onClick={() => handleSelectStock(stock.ticker)}>
                      <input
                        className='cursor-pointer'
                        type="checkbox"
                        checked={isSelected}
                    ...
                    <td className="py-0 px-0 border-b border-gray-300 text-gray-400 text-right cursor-pointer" onClick={() => handleSelectStock(stock.ticker)}>{index +1}</td>
                  <td className="py-1 px-2 border-b border-gray-300 cursor-pointer" onClick={() => onClickStock(stock.ticker)}>{stock.ticker}</td>
                  
                  <td className="py-1 px-2 border-b border-gray-300 cursor-pointer" onClick={() => onClickStock(stock.ticker)}>{stock.name}</td>
                  <td className="py-1 px-2 border-b border-gray-300 cursor-pointer" onClick={() => onClickStock(stock.ticker)}>{stock.esg.toFixed(2)}</td>
                <td className="py-1 px-2 border-b border-gray-300 cursor-pointer" onClick={() => onClickStock(stock.ticker)}>{stock.compatibility_score.toFixed(0)}%</td>
                <td className="py-1 px-2 border-b border-gray-300 cursor-pointer" onClick={() => onClickStock(stock.ticker)}>
                  {isInPortfolio ? (
  ...
  <button onClick={() => setSelectedStocks([])} className=" text-xs hover:underline text-gray-500 hover:opacity-75">
